...
class MyModel(nn.Module):
    def __init__(self, input_dim, z_dim, n_clusters, encodeLayer=[], decodeLayer=[], 
        ...
        # 添加源头GAN网络
        self.source_generator = Generator(z_dim, input_dim)  # 生成器
        self.source_discriminator = Discriminator(input_dim)  # 判别器
        ...
        self.contrastive_loss = get_contrastive_loss('multi_positive')
        self.to(device)
        ...
        self.tail_discriminator = nn.Sequential(
            nn.Linear(z_dim, 128),
            nn.ReLU(),
            nn.Linear(128, 1),
            nn.Sigmoid()
    ...
    def forward(self, x):
        ###作用是对数据进行预训练，得到相应的中间变量，z0, q是用原始的x得到的，_mean, _disp, _pi是用加了噪声的x得到的。
        ###返回值：
        ###z0:原始数据的潜在表示
        ###q:软分配结果
        ###_mean, _disp, _pi重构参数
        # 源头GAN的生成过程
        noise = torch.randn(x.size(0), self.z_dim).to(x.device)
        fake_data = self.source_generator(noise)
        ...
        return z0, q, _mean, _disp, _pi, fake_data
    ...
    def encodeBatch(self, X, batch_size=256):####encodeBatch的作用是对输入进行编码得到编码的嵌入向量，输入就是原始的x，
        ...
        for batch_idx in range(num_batch):###对每一批
            ...
            z,_, _, _, _, _ = self.forward(inputs)###对数据进行编码2000-256-64-32得到z
    ...
    def pretrain_autoencoder(self, x, X_raw, size_factor, batch_size=64, lr=0.001, epochs=400, ae_save=True, ae_weights='AE_weights.pth.tar'):
        '''
        对 autoencoder 进行预训练
        '''
        use_cuda = torch.cuda.is_available()
        if use_cuda:
            self.cuda()
        # 这个dataset是一个TensorDataset对象，包含三个tensor：
        # 第一个是x，形状为(num_cells, num_features)，比如268*2000，表示预处理后的输入数据；
        # 第二个是X_raw，形状同上，也是268*2000，表示原始输入数据；
        # 第三个是size_factor，形状为(num_cells,)，比如268，表示每个细胞的size factor（归一化因子）。
        ...
        d_optimizer = optim.AdamW(self.source_discriminator.parameters(), lr=lr)
        g_optimizer = optim.AdamW(self.source_generator.parameters(), lr=lr)
        ...
        for epoch in range(epochs):
            for batch_idx, (x_batch, x_raw_batch, sf_batch) in enumerate(dataloader):
                ...
                # 真实数据的损失
                real_output = self.source_discriminator(x_tensor)
                d_loss_real = criterion(real_output, real_label)
                
                # 生成数据的损失
                noise = torch.randn(batch_size, self.z_dim).cuda()
                fake_data = self.source_generator(noise)
                fake_output = self.source_discriminator(fake_data.detach())
                ...
                fake_output = self.source_discriminator(fake_data)
                g_loss = criterion(fake_output, real_label)
                g_loss.backward()
                g_optimizer.step()
                
                # 训练自编码器
                _, _, mean_tensor, disp_tensor, pi_tensor, _ = self.forward(x_tensor)
                zinb_loss = self.zinb_loss(x=x_raw_tensor, mean=mean_tensor, 
                                         disp=disp_tensor, pi=pi_tensor, scale_factor=sf_tensor)
                ...
                if batch_idx % 10 == 0:
                    print('Pretrain epoch [{}/{}], ZINB loss:{:.4f}, D loss:{:.4f}, G loss:{:.4f}'.format(
                        epoch+1, epochs, zinb_loss.item(), d_loss.item(), g_loss.item()))
    ...
    def fit(self, X, X_raw, sf, y=None, lr=1., batch_size=256, num_epochs=10, save_path=''):
        ...
        d_optimizer = optim.Adadelta(self.source_discriminator.parameters(), lr=lr)
        g_optimizer = optim.Adadelta(self.source_generator.parameters(), lr=lr)
        
        criterion = nn.BCELoss()
        
        print("Initializing cluster centers with kmeans.")
        # 你可以直接通过 self.n_clusters 来获取聚类的类别数目，比如：
        # kmeans = KMeans(self.n_clusters, n_init=20)
        # 只要实例化了这个类，self.n_clusters 就可以被调用，无需额外操作。
        # 例如：model = MyModel(n_clusters=10)，之后 model.n_clusters 就是 10。
        # 这种写法是 Python 的类属性访问语法，self 代表类的实例，self.n_clusters 就是实例的 n_clusters 属性
        kmeans = KMeans(self.n_clusters, n_init=20)  # 定义聚类方法是kmeans，类别数目由self.n_clusters指定，选20次最优的
        ...
        # 添加尾部GAN优化器
        tail_g_optimizer = optim.Adadelta(self.tail_generator.parameters(), lr=lr)
        tail_d_optimizer = optim.Adadelta(self.tail_discriminator.parameters(), lr=lr)
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx in range(num_batch):
                ...
                # 1. 训练判别器
                d_optimizer.zero_grad()
                real_label = torch.ones(len(inputs), 1).cuda()
                fake_label = torch.zeros(len(inputs), 1).cuda()
                
                real_output = self.source_discriminator(inputs)
                d_loss_real = criterion(real_output, real_label)
                
                noise = torch.randn(len(inputs), self.z_dim).cuda()
                fake_data = self.source_generator(noise)
                fake_output = self.source_discriminator(fake_data.detach())
                d_loss_fake = criterion(fake_output, fake_label)
                ...
                # 3. 训练自编码器和聚类
                optimizer.zero_grad()
                z, qbatch, meanbatch, dispbatch, pibatch, fake_data = self.forward(inputs)
                ...
                # 计算对比损失
                contrast_loss = self.contrastive_loss(z, labels=target.argmax(dim=1))
                
                # 合并所有损失
                total_loss = cluster_loss + recon_loss + 0.1 * (d_loss + g_loss) + 0.1 * contrast_loss
                total_loss.backward()
                optimizer.step()
                ...
                gan_loss_val += (d_loss + g_loss).data * len(inputs)
                contrast_loss_val += contrast_loss.data * len(inputs)
                train_loss = cluster_loss_val + recon_loss_val + gan_loss_val + contrast_loss_val
                
                # 在自编码器训练后添加尾部GAN训练
                
                # 1. 训练尾部判别器
                tail_d_optimizer.zero_grad()
                
                # 获取真实的隐层表示
                z_real = z.detach()  # 从forward返回的z
                
                # 生成假的隐层表示
                z_noise = torch.randn(len(inputs), self.z_dim).cuda()
                z_fake = self.tail_generator(z_noise)
                
                # 计算判别器损失
                tail_d_real = self.tail_discriminator(z_real)
                tail_d_fake = self.tail_discriminator(z_fake.detach())
                
                tail_d_loss = criterion(tail_d_real, real_label) + criterion(tail_d_fake, fake_label)
                tail_d_loss.backward()
                tail_d_optimizer.step()
                
                # 2. 训练尾部生成器
                tail_g_optimizer.zero_grad()
                
                # 重新生成假样本
                z_fake = self.tail_generator(z_noise)
                tail_d_fake = self.tail_discriminator(z_fake)
                
                # 计算生成器损失
                tail_g_loss = criterion(tail_d_fake, real_label)
                
                # 添加聚类结构保持损失
                fake_q = self.soft_assign(z_fake)
                structure_loss = self.cluster_loss(target, fake_q)
                
                total_tail_g_loss = tail_g_loss + 0.1 * structure_loss
                total_tail_g_loss.backward()
                tail_g_optimizer.step()
                
                # 更新总损失
                total_loss += 0.1 * (tail_d_loss + total_tail_g_loss)
                gan_loss_val += (tail_d_loss + total_tail_g_loss).data * len(inputs)

            print("#Epoch %3d: Total: %.4f Clustering Loss: %.4f ZINB Loss: %.4f GAN Loss: %.4f Contrast Loss: %.4f" % (
                epoch + 1, train_loss / num, cluster_loss_val / num, 
                recon_loss_val / num, gan_loss_val / num, contrast_loss_val / num))
...
data_name = 'muraro'
data_path = 'E:\\Study\\ganEx\\upload\\klug\\atlas_2000_sample.txt' 
label_path = 'E:\\Study\\ganEx\\upload\\klug\\atlas_lung_truelabels1_simple.csv' 
